...
\noindent for $i=1,2,\dots,n$. We want to find a simple algorithm to solve this set of equations. To do this, we can rewrite Eq.~\ref{eq:tridiagonal_system} by a method of elimination, which will lead to a system where there is only one unknown. The idea is to subtract one row with a scalar multiple of another.
...
\noindent where the boundary conditions have been applied to simplify the equation. \\
